...
const Nav: React.FC<NavProps> = memo(({ count, onCartClick }) => {
  ...
  return (
    <>
      ...
      {/* ── Main header ── */}
      <header className={`fixed top-9 inset-x-0 z-40 transition-all duration-300 ${scrolled
        ...
        <div className="max-w-7xl mx-auto px-5 lg:px-8 flex items-center h-[68px] gap-5">
          ...
          <nav className="hidden lg:flex items-center gap-0.5 ml-4">
            ...
            {[
              ...
              <Link key={to} to={to}
                className={`px-3.5 py-2 rounded-xl text-[13px] font-medium transition-all ${isActive(to)
        ...
        <div className={`absolute top-0 left-0 h-full w-[290px] bg-white flex flex-col transition-transform duration-400 ease-out ${drawerOpen ? 'translate-x-0' : '-translate-x-full'}`}
          ...
          <div className="flex-1 overflow-y-auto py-3 px-3 space-y-0.5">
            {[
              ...
              <Link key={to} to={to}
                className={`block px-3 py-3 rounded-xl text-[13px] font-medium transition-colors ${isActive(to) ? 'bg-[#eaf0e2] text-[#2e3c27]' : 'text-[#3a3830] hover:bg-[#f2f7ec]'
                  }`}>
                {label}
              </Link>
            ))}
